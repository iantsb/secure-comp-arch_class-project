...
For this system, the model is considered to be a black box to an attacker since the confidentiality of the weights and other model IP is our primary goal. Additionally, the bus is assumed to be unsafe and vulnerable to physical probing by an attacker to snoop weight data. We also consider an attacker that can induce targeted or random bit flips on the weight data as it is transmitted across the bus. Entirely attacker generated messages are not considered, therefore our mitigation does not defend against replay attacks. For the purposes of this work, the CPU and accelerator are assumed to be safe and data stored on either is trusted. 

\section{Experiment}
Please find all python code and other source materials used for the experiment here: \url{https://github.com/iantsb/secure-comp-arch_class-project}
...
ReRAM cells have been examined by several works as a PUF source. These works are further examined in Section \ref{subsec:PUFS} but show that ReRAM can be used as reliable and secure PUFs in a variety of manners. Common across these works, however, is the use of ReRAM PUFs without utilizing the ReRAM for memory. As the PUF designs are based on physical properties of the cell, the wear placed on the cells due to read/write operations change these properties and alters PUF behavior. As examined in Section \ref{subsec:hiding}, this wear on the read/write times of the cells happens in a predictable manner and has reliable effects on the read/write time.
...
\section{Evaluation}
\subsection{PUF Evaluation}



Two evaluations were done for the PUF generation. The first
examined drift of the key generation over time due to the wear placed on the
ReRAM. A simple wear model was considered, in which every key generation (SET race)
altered the set time by a small amount. This was based below in \ref{subsec:hiding}, where the
stress for a SET race was demonstrated. Three results were plotted to show the key
Hamming distance (relative to the initial key as a baseline) over time: the raw
value of a single-shot key generation, of a round-based key generation, and
then the hashed value of a single-shot key generation. These results are displayed
in Figure \ref{PUF_drift}. These results demonstrate that over time, the raw value of the
key moves in a predictable manner, and the shift is observably different
between single-shot and round-based encryption. The hashed key does not obey a
predictable change, as the hashing function is non-linear (SHA256) and the
variation between steps is random. However, all three methods display an
initial time before changes are observed (\~40 ns in this model), as well as key
saturation as the offset approaches 100 ns. Interestingly, the saturation
happens later for round-based generation, which is counter-intuitive. It would
be expected that, due to the increased wear, saturation would happen much
earlier. This does not happen as the evaluated round-based scheme generates 128
bits per round, requiring a majority vote procedure to determine the key. This
majority voting introduces an increased layer of entropy, delaying saturation
in the key values.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{time_change.png}
    \caption{PUF Drift over time}
    \label{PUF_drift}
\end{figure}

The second evaluation examined PUF strength and
speed for different generation configurations. The number of rows utilized
varied from 4-256 and the number of banks varied from 1-64, creating 16,128
data points to consider. These were then evaluated for generation speed, bit
mean, and inter-challenge Hamming distance. The generation speed was based on
literature values for ReRAM write speed, as well as an estimate of the array
access time. The bit mean represents the average distribution of zeros and ones
in the generated keys for a given configuration. This means that 0.5 is the
ideal bit mean, representing an unbiased key (no bias towards one or zero). The
inter-challenge Hamming distance is the average Hamming distance (HD) (number
of changing bits) between different challenges for a given array configuration.
This means that, for a 128-bit key (generated here), the optimal HD is 64,
representing that half the bits change between challenges and the challenges
are uncorrelated. Two scatter plots are shown in Figure \ref{scatter} (a) and (b) with a
small subset of the data, demonstrating the result space for this evaluation.
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}{0.9\linewidth}
        \centering
        \includegraphics[width=\linewidth]{strength_bal.png}
        \caption{Bit Mean Scatter Plot}
    \end{subfigure}
    ...
    \begin{subfigure}{0.9\linewidth}
        \centering
        \includegraphics[width=\linewidth]{strength_hd.png}
        \caption{Hamming Distance Scatter Plot}
    \end{subfigure}

    \caption{Initial Plotted Results}
    \label{scatter}
\end{figure}


The best results are presented in Table \ref{puf_best}, comparing results for various
configurations. For the calculation of access time, the raw access time as well
as the adjusted access time is reported. All configurations were required to
use the same number of challenge bits, set here to be 32. For configurations
that cannot natively accept a 32-bit challenge, a round-based scheme is
required to introduce additional challenge bits. For example: in a 16-row,
1-bank configuration, it only requires 4-bits to determine the row accessed. For
a 32-bit challenge, this requires 8 rounds of generation with a separate 4 bits
used every time to meet the 32-bit requirement. Conversely, for a 16-row, 8-bank
configuration, each bank utilizes a different 4 bits of the challenge to
natively accept a 32-bit challenge. The best results are those with the
shortest adjusted access time as well as the bit mean and inter-HD closest to optimal.

\begin{table}[h!]
\centering
\begin{tabular}{c c c c c c}
\hline
\# Rows & \# Banks & Bit Mean & HD (avg) & Raw Time & Adjusted Time \\
\hline
256 & 1 & 0.487 & 51.47 & 140 ns & 560 ns \\
16  & 1 & 0.487 & 51.47 & 140 ns & 1120 ns \\
16  & 8 & 0.487 & 46.47 & 1120 ns & 1120 ns \\
\hline
\end{tabular}
\caption{PUF geometry comparison}
\label{puf_best}
\end{table}

The general finding of this evaluation was that round-based schemes performed
far better than single-shot schemes. This is shown in Figure \ref{bank_count} (a) and (b),
showing that Hamming distance and bit mean are generally closer to the optimal value
with fewer banks (the dashed line represents the optimal value). Figure \ref{bank_count} (c)
shows the generation time for number of banks (both raw and adjusted values),
demonstrating that the generation time is directly correlated with bank count. This
means that it is optimal to generate the key in rounds using a minimal number
of banks, as the tradeoff between generation time and key strength is not
directly correlated.
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{bank_HD.png}
        \caption{Hamming Distance vs #Banks}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{bank_mean.png}
        \caption{Bit Mean vs #Banks}
    \end{subfigure}
 
    \vspace{0.5em} % small vertical spacing between rows

    \begin{subfigure}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{bank_time.png}
        \caption{Access Time vs #Banks}
    \end{subfigure}
 
    \caption{Results versus Bank Count}
    \label{bank_count}
\end{figure}
...
\subsection{Weight Integrity}
For an integrated functional simulation in python, the SHA256 class is used from the pycryptodome library. The integrity check was verified with a simple bit flipping function that XORs the ciphertext with a 64-bit mask where 1 bit is randomly set using the numpy random module. See Appendix \ref{sec.bit_flip_method} for the detailed code.

To obtain a power analysis of the SHA256 module, RTL from an open source github project\cite{SHA256} was synthesized with Vivado for an Artix 7 FPGA development board. This Verilog implementation of SHA256 has a 512-bit block input with a 256-bit disgest and executes 64 rounds of hashing, one round per cycle. Clockspeed was set to 50MHz. Using this test setup, the reported power of the RTL is detailed in Table \ref{power report}, notably the total is only 0.557W. The full power report is available in our github repository in sha256\_power\_report.txt. 
...
a ReRAM cell changes with each write cycle, shown in Figure \ref{hiding}. This occurs due to oxygen vacancies in the oxide layer used to construct the conductive
...
that, once the set/reset characteristics of a cell have been altered, they can reliably determine cells with more or less "wear". By
setting a point at which set/reset times can be read as binary "1" or "0", cells can intentionally be worn down to encode values in the
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{hiding_reram}
  \caption{ReRAM Wear due to SET Procedure}
  \label{hiding}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\appendices
\section{Bit Flip Method}
\label{sec.bit_flip_method}
\begin{lstlisting}[frame=single, language=Python]
def fault_injection(enc_weight: float) 
-> float:
  flipped_weight: float = 0.0
  rng = np.random.default_rng()
  rint = rng.integers(low=0, high=63, 
  size=1)

  #change enc_weight to 64b integer
  bits ^= int(1 << rint[0])
  #convert bits to float and 
  #store in flipped_weight
